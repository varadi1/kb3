...
class DocProcessor extends BaseProcessor_1.BaseProcessor {
    constructor(maxTextLength = 1000000) {
        super([IUrlDetector_1.ContentType.DOC], maxTextLength);
    }
    async performProcessing(content, _contentType, options) {
        ...
        try {
            ...
            return this.createProcessedContent(text, title, metadata, [], // No image extraction for DOC files in this simple implementation
            links, [], // No table extraction for DOC files in this simple implementation
            structure);
        }
        catch (error) {
    ...
    extractDocMetadata(buffer, text, _options) {
        const metadata = {
            contentType: IUrlDetector_1.ContentType.DOC,
