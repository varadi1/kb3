...
class PlaywrightScraper extends BaseScraper_1.BaseScraper {
    browserInstance = null;
    contextInstance = null;
    constructor() {
        super(IScraper_1.ScraperType.PLAYWRIGHT, {
            javascript: true,
            cookies: true,
            proxy: true,
            screenshot: true,
    ...
    extractPlaywrightParams(options) {
        const defaults = {
            headless: true,
        ...
        if (options.cookies) {
            defaults.cookies = options.cookies;
        }
        if (options.proxy) {
    ...
    async launchBrowser(playwright, params) {
        if (this.browserInstance) {
    ...
    async createContext(browser, params) {
    ...
    async configurePage(page, params) {
    ...
    async navigateToUrl(page, url, params) {
    ...
    async waitForContent(page, params) {
        ...
        await page.evaluate(() => {
            return new Promise((resolve) => {
                ...
                const timer = setInterval(() => {
                    const scrollHeight = (typeof document !== 'undefined' ? document.body.scrollHeight : 1000);
    ...
    async clickElements(page, selectors) {
    ...
    async extractContent(page, _params) {
        const html = await page.content();
        return Buffer.from(html, 'utf-8');
    }
    async buildMetadata(page, response, params, startTime) {
