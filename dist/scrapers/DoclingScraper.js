...
exports.DoclingScraper = void 0;
const BaseScraper_1 = require("./BaseScraper");
const IScraper_1 = require("../interfaces/IScraper");
const PythonBridge_1 = require("./PythonBridge");
const path = __importStar(require("path"));
class DoclingScraper extends BaseScraper_1.BaseScraper {
    pythonBridge;
    wrapperPath;
    documentCache = new Map();
    constructor() {
        super(IScraper_1.ScraperType.DOCLING, {
            javascript: false,
            cookies: false,
            proxy: false,
            screenshot: false,
    ...
    async downloadDocument(url, params) {
        // Check cache first
    ...
    async extractContent(result, _params) {
        let content;
    ...
    tableToCSV(table) {
        ...
        return table.rows.map((row) => row.map((cell) => `"${cell.replace(/"/g, '""')}"`).join(',')).join('\n');
    }
    tableToHTML(table) {
        if (!table.rows || table.rows.length === 0) {
            return '<table></table>';
    ...
    buildMetadata(result, params, startTime) {
        ...
        if (result.tables && result.tables.length > 0 && metadata.scraperMetadata) {
            metadata.scraperMetadata.tables = result.tables.map((table) => ({
        ...
        if (result.figures && result.figures.length > 0 && metadata.scraperMetadata) {
            metadata.scraperMetadata.figures = result.figures.map((figure) => ({
        ...
        if (result.embedded_files && result.embedded_files.length > 0 && metadata.scraperMetadata) {
            metadata.scraperMetadata.embeddedFiles = result.embedded_files.map((file) => ({
                name: file.name,
                size: file.size,
                type: file.type
            }));
    ...
    canHandle(url) {
        ...
        const hasDocumentExtension = documentExtensions.some(ext => lowercaseUrl.includes(ext));
        ...
        const matchesDocumentPattern = documentPatterns.some(pattern => pattern.test(url));
    ...
    getBatchSize(options) {
        const _params = options.scraperSpecific;
...
exports.DoclingScraper = DoclingScraper;
//# sourceMappingURL=DoclingScraper.js.map